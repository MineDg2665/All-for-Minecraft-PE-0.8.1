...
Changelog:
0.3.9_1 (Tema1d's update)
* alias for /l (/login) and /reg (/register)

0.3.9Translate (NCDev's update)
* PHP8 Compability

0.3.8Translate (Tema1d's update)
* Переведено на русский язык.

0.3.8Nostalgic (ArkQuark's update)
...
class SimpleAuth implements Plugin{
	...
	public function init(){
		...
		$cfgpath = $this->api->plugin->configPath($this)."players/";
		if(!is_dir($cfgpath)) mkdir($cfgpath);
		if(file_exists($this->api->plugin->configPath($this)."players.yml")){			
			...
			console("[Инфо] [SimpleAuth] Импорт старого формата в новый формат...");
		...
		//console("[Инфо] SimpleAuth загружен!");
	}
			...
			case "simpleauth":
				...
				switch(strtolower(array_shift($params))){
					...
					default:
						...
						$output .= "/simpleauth unreg <player>: Removes a player from the database.\n";
				}
				break;
			case "unreg":
				...
				if($d !== false and $d->get("hash") === $this->hash($issuer->iusername, implode(" ", $params))){
					...
					$this->logout($issuer);
					$output .= "[RPGCraft] You have successfully deleted your account.\n";
				}else{
					$output .= "[RPGCraft] Authentication error.\n";
				}
				break;
		}
		return $output;
	...
	public function checkTimer(){
		if($this->config->get("allowRegister") !== false and ($this->lastBroadcast + $this->config->get("messageInterval")) <= -1){
			$broadcast = true;
		}else{
			$broadcast = false;
		}
		...
		foreach($this->sessions as $CID => $timer){
			if($timer !== true and $timer !== false and isset($this->server->clients[$CID]) and ($this->server->clients[$CID] instanceof Player)){				
				if($broadcast === true){
					...
					if($d === false){
						$this->server->clients[$CID]->sendChat("[RPGCraft] You must register using: /reg <password>");
					}else{
						$this->server->clients[$CID]->sendChat("[RPGCraft] You must log in using: /l <password>");
					}
				}
	...
	public function login(Player $player){
		...
		$player->sendChat("[RPGCraft] You are logged in! HAVE A GOOD GAME!");
		console("[SimpleAuth] " . $name . " logged in");
	...
	public function register(Player $player, $password){	
		$d = $this->getData($player->iusername);
		if($d === false){
			$pldir = $this->api->plugin->configPath($this)."players/".$player->iusername[0]."/";
			if(!is_dir($pldir)){
				@mkdir($pldir);
			}
			$d = new Config($pldir.$player->iusername.".yml", CONFIG_YAML, array());
		...
		switch($event){
			...
			case "player.spawn":
				if(!isset($this->sessions[$data->CID]) or $this->sessions[$data->CID] !== true){
					...
					$data->blocked = true;
					$data->sendChat("	========================================	\n||           WELCOME TO THE RPGCraft SERVER!!!         ||\n||                                                              ||\n||         BE SURE TO JOIN DISCORD SERVER!           ||\n|| Discord: https://discord.com/invite/nTaEVhWGXx ||\n||   Site: https://worldpe.rf.gd (buy donate pls!)    ||\n	========================================	\n[WorldPE] This server uses SimpleAuth to protect your account!");
					if($this->config->get("allowRegister") !== false){
						...
						if($d === false){					
							$data->sendChat("[RPGCraft] You must register using: /reg <password>");
						}else{
							$data->sendChat("[RPGCraft] You must log in using: /l <password>");
						}
					}
				}
				break;
			...
			case "console.command":
				if(($data["issuer"] instanceof Player) and $this->sessions[$data["issuer"]->CID] !== true){
					if($data["cmd"] === "l" || $data["cmd"] === "login" and $this->checkLogin($data["issuer"], implode(" ", $data["parameters"])) === true){
						$this->login($data["issuer"]);
						return true;
					}elseif($data["cmd"] === "reg" || $data["cmd"] === "register" and strlen(implode(" ", $data["parameters"])) < $this->config->get("minPasswordLength")){
						$data["issuer"]->sendChat("[RPGCraft] Password is too short.");
						return true;
					}elseif($this->config->get("allowRegister") !== false and $data["cmd"] === "reg" || $data["cmd"] === "register" and $this->register($data["issuer"], implode(" ", $data["parameters"])) === true){
						$data["issuer"]->sendChat("[RPGCraft] You have successfully registered.");
						$this->login($data["issuer"]);
						return true;
					}elseif($data["cmd"] === "l" || $data["cmd"] === "login" or $data["cmd"] === "reg" || $data["cmd"] === "register"){
						$data["issuer"]->sendChat("[RPGCraft] Wrong password!");
						return true;
					}
					return false;
				}
