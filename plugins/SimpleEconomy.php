...
class SimpleEconomy implements Plugin{
    ...
    public function onBlockBreak($data, $event){
        ...
        $block = $data["target"];
        $oreIds = [14, 15, 16, 21, 56, 73, 129];

        if(in_array($block->getID(), $oreIds)){
            $min = (int)$this->config->get("pay-amount-min", 3);
            $max = (int)$this->config->get("pay-amount-max", 10);
            $amount = mt_rand($min, $max);
            $this->addBalance($player->username, $amount);
            $player->sendChat("[SimpleEconomy] You received $amount {$this->currency} for mining ore.");
        }
    }

    public function onEntityDeath($data, $event){
        $entity = $data["entity"];
        $killer = isset($data["killer"]) ? $data["killer"] : null;
        if(!($killer instanceof Player)) return;

        $peacefulIds = [10, 11, 12, 13, 31, 32, 33, 34, 35];
        $hostileIds = [50, 51, 52, 53, 54, 55, 56, 57, 58];
